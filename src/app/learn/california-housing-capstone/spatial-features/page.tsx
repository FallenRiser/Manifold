...
export const metadata = {
  ...
  description:
    "Upgrade 1: the linear model can't bend to California's geography, so we engineer features that do the bending for it — distance to the coast and to major metros, plus geographic region clusters. Result: R² 0.653 → 0.672.",
};
...
export default function SpatialFeaturesPage() {
  return (
    <article>
      <LessonHeader
        chips={[{ label: "Capstone", color: "var(--c-regression)" }, { label: "Upgrade 1 · Geography", color: "var(--c-metrics)" }]}
        time="about 8 minutes"
        title={<>Upgrade 1 · Spatial feature engineering</>}
        intro={<>
      ...
      <div className="lesson">
        ...
        <GuessSlider
          prompt={<>The baseline sits at R² 0.653. Three distances plus 12 region intercepts, model still linear. How much R² does the spatial upgrade buy?</>}
          min={0}
          max={0.15}
          step={0.001}
          start={0.05}
          ...
          decimals={3}
          signed
          reveal={<>The lift is <strong>+0.019</strong> (0.653 → 0.672) — real, repeatable, and smaller than most people guess. Hand-engineered features are still additive and monotonic; they bend the linear model, they can&rsquo;t carve the price surface. Remember this gap when the trees show up.</>}
        />
        <CodeBlock fromScratch={code} withLibrary={code} />
        <CodeOutput>{`dist_coast correlation with price:  -0.429   (closer coast -> pricier)

5-fold CV  (ridge)
  base 10 features            RMSE 0.681   R2 0.653
  + spatial features          RMSE 0.662   R2 0.672     <- lift

added: dist_coast, dist_sf, dist_la, + 12 region one-hots`}</CodeOutput>

        <h2>The result, read honestly</h2>
        <p>
          R² rises from <strong>0.653 to 0.672</strong> and RMSE falls from 0.681 to 0.662 — a real, repeatable
        ...
        <Callout color="var(--c-regression)" title={<>Feature engineering is domain knowledge as code</>}>
        ...
        <PrevNext prev={{ href: "/learn/california-housing-capstone/diagnostics", label: <>← Diagnostics: what it misses</> }} next={{ href: "/learn/california-housing-capstone/censored-regression", label: <>Next up · Upgrade 2: Tobit &amp; censored regression →</> }} />
      </div>
    </article>
  );
}
